                            ...
                            <div class="border shadow-none card">
                                <div class="p-0 card-body">
                                    ...
                                    <div class="card-list">
                                        ...
                                        <b>
                                            @if (app()->getLocale() == 'en')
                                            {{ @$serviceRequest->propertyType->name }}
                                            @else
                                            {{ @$serviceRequest->country->name_ar }}
                                            @endif
                                        </b>
                                    ...
                                    <div class="card-list">
                                        <span>@lang('Sector Type')</span>
                                        <b>
                                            @if ($serviceRequest->sectors)
                                                @foreach (json_decode($serviceRequest->sectors) as $sector)
                                                    {{  $sector }},
                                                @endforeach
                                            @endif
                                        </b>
...
@push('breadcrumb-plugins')
    <div class="flex-wrap gap-2 d-flex">
        <a href="{{ route('admin.social.service.request.index') }}" class="btn btn-primary"><i
                class="bi bi-arrow-clockwise pe-1"></i>@lang('Back')</a>

        <a href="{{ route('admin.social.service.request.status', [$serviceRequest->id, Status::REVIEW]) }}" class="btn btn-success"><i
                class="bi bi-check2 pe-1"></i>@lang('Approved')</a>
        <a href="{{ route('admin.social.service.request.status', [$serviceRequest->id, Status::REJECT]) }}" class="btn btn-danger"><i
