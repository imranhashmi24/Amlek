@extends('web.layouts.frontend', ['title' => $auction->category->name . ' Auction Details'])
			...
			<div class="row g-4">
				...
				<div class="col-xl-8 col-lg-7">
					...
					<div class="car-gallery mb-4">
						<div class="main-image mb-2">
							<img src="{{ getImage(getFilePath('auction_thumb') . '/' . $auction->thumb_image, getFileSize('auction_thumb')) }}"
								alt="{{ $auction->title }}" class="img-fluid rounded"
								onerror="this.src='https://via.placeholder.com/1000x500/eeeeee/999999?text=No+Image'">
						</div>
						<div class="row g-2 thumb-gallery">
							@foreach ($auction->images as $image)
								<div class="col-3">
									<img src="{{ getImage(getFilePath('auction') . '/' . $image->image, getFileSize('auction')) }}" alt="Thumb"
										class="img-fluid rounded">
								</div>
							@endforeach
							@if ($auction->images->count() < 4)
								@for ($i = $auction->images->count(); $i < 4; $i++)
									<div class="col-3">
					...
					<div class="car-header-info mb-4">
						<h3 class="car-title fw-bold">{{ app()->getLocale() == 'en' ? $auction->title : $auction->title_ar }}</h3>
						@php
							$subtitleParts = [];
							if ($auction->mileage) {
						...
						@if (!empty($subtitleParts))
							<p class="text-muted small">{{ implode(', ', $subtitleParts) }}</p>
						@endif
					</div>
						...
						<table class="table table-bordered align-middle">
							<tbody>
								...
								@foreach ($specs as $label => $value)
									@if ($value)
										<tr>
											<td class="bg-light fw-semibold text-muted label-col">{{ $label }}</td>
											<td>{{ $value }}</td>
											@if ($loop->iteration % 2 == 1 && !$loop->last)
												...
												@endphp
												@if ($nextValue)
													<td class="bg-light fw-semibold text-muted label-col">{{ $nextLabel }}</td>
													<td>{{ $nextValue }}</td>
												@else
													<td></td>
													<td></td>
					...
					<div class="car-details-sections">
						@if ($auction->highlights)
							<div class="detail-section mb-4">
								<h5 class="fw-bold mb-3">@lang('Highlights')</h5>
								<ul class="custom-check-list">
									@foreach (explode("\n", $auction->highlights) as $line)
										@if (trim($line))
											<li>{{ trim($line) }}</li>
										@endif
									@endforeach
						...
						@if ($auction->other_items)
							<div class="detail-section mb-4">
								<h5 class="fw-bold mb-3">@lang('Other Items Included in Sale')</h5>
								<ul class="custom-check-list">
									@foreach (explode("\n", $auction->other_items) as $line)
										@if (trim($line))
											<li>{{ trim($line) }}</li>
										@endif
									@endforeach
						...
						@if ($auction->seller_notes)
							<div class="detail-section mb-4">
								<h5 class="fw-bold mb-3">@lang('Seller Notes')</h5>
								<ul class="custom-check-list">
									@foreach (explode("\n", $auction->seller_notes) as $line)
										@if (trim($line))
											<li>{{ trim($line) }}</li>
										@endif
									@endforeach
				...
				<div class="col-xl-4 col-lg-5">
					<div class="bid-action-box mb-4">
						<p class="text-muted small mb-1">@lang('Current Bid')</p>
						<h2 class="bid-price fw-bold mb-4">{{ showAmount($auction->starting_price ?? 0) }}</h2>
						...
						<p class="text-muted small mb-1">@lang('Ending'):</p>
						<p class="fw-bold mb-3">
							@if ($endDate->gt($now))
								{{ $diff->d }} @lang('Days') | {{ $diff->h }} @lang('Hours') | {{ $diff->i }}
								@lang('Minutes')
							@else
								@lang('Auction ended')
							@endif
						...
						@if ($endDate->gt($now))
							<a href="{{ route('bidding.request.page', $auction->slug) }}"
								class="btn btn-primary w-100 btn-lg mb-3 place-bid-btn">@lang('Place Bid')</a>
						@else
							<button class="btn btn-secondary w-100 btn-lg mb-3" disabled>@lang('Closed')</button>
						@endif

						<div class="watching-bidders d-flex align-items-center">
							<div class="avatar-group me-2">
								@foreach ($auction->biddings->take(4) as $bid)
									<img src="{{ getImage(getFilePath('userProfile') . '/' . $bid->user->image) }}"
										class="rounded-circle border border-white" alt="bidder">
								@endforeach
							</div>
							<span class="small fw-semibold">@lang('Watching Bidder')</span>
						</div>
					</div>

					<!-- Similar Auctions (from same category) -->
					@if (isset($relatedAuctions) && $relatedAuctions->count())
						<div class="row g-3">
							@foreach ($relatedAuctions->take(2) as $related)
								<div class="col-6">
									<div class="auction-card">
										<div class="auction-img">
											<img
												src="{{ getImage(getFilePath('auction_thumb') . '/' . $related->thumb_image, getFileSize('auction_thumb')) }}"
												alt="{{ $related->title }}" onerror="this.src='https://via.placeholder.com/300x150/eeeeee/999999'">
										</div>
										<div class="auction-content">
											<h6 class="auction-title">
												{{ Str::limit(app()->getLocale() == 'en' ? $related->title : $related->title_ar, 40) }}</h6>
											<div class="auction-feature">
												<div class="feature-row-full"><i class="bi bi-check2"></i> @lang('Comprehensive Inspection')</div>
												<div class="feature-row-full"><i class="bi bi-clock"></i> @lang('Live Bidding')</div>
												<div class="feature-row-full"><i class="bi bi-truck"></i> @lang('Delivery Options')</div>
											</div>
											<div class="auction-footer">
												<div class="price-info">
													<span class="starting-label">@lang('Starting at'):</span>
													<div class="auction-price">{{ showAmount($related->starting_price ?? 0) }}</div>
												</div>
												<a href="{{ route('auction.details', $related->slug) }}"
													class="btn-bid text-center mt-2 d-block">@lang('Bid Now')</a>
											</div>
										</div>
									</div>
								</div>
