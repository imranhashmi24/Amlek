...
class ApiController extends Controller
{
    ...
    {
        $o_countries = Country::orderByRaw('ISNULL(sort_order), sort_order')->get();
        $countries = sortOrder($o_countries);
        return $this->successResponse($countries, 'countries');
    }
    ...
    public function assetsAndLiabilities()
    {
        try {
            ...
            if (!$liabilities) {
                return $this->serverError('No liabilities found');
            }
    
            $filteredElements = [];

            foreach ($liabilities as $element) {
                $dataValues = $element['data_values'];

                if (is_object($dataValues) && isset($dataValues->title, $dataValues->title_ar, $dataValues->slider)) {
                    $filteredElements[] = [
                        'title' => $dataValues->title,
                        'title_ar' => $dataValues->title_ar,
                        'slider' => getImage('assets/images/frontend/offer_banner/' . $dataValues->slider),
                    ];
                }
            }
    ...
    {
        $floorPlan = \App\Models\Frontend::where('id', $id)->first();
        
        // $data = $floorPlan->data_values;
        
        $data[] = [
            'id'    => $floorPlan->id,
            'title' => $floorPlan['data_values']->title,
            'title_ar' => $floorPlan['data_values']->title_ar,
            'description' => $floorPlan['data_values']->description,
            'description_ar' => $floorPlan['data_values']->description_ar,
            'plan_image' => getImage('assets/images/frontend/floor_plan/' . $floorPlan['data_values']->plan),
        ];
        return $this->successResponse($data);
    }
    ...
    public function floorPlanRequestStore(Request $request)
    {
       ...
       if($validator->fails()){
        ...
        try{
            $floorPlanRequest = new \App\Models\FloorPlanRequest();
            $floorPlanRequest->plan = $request->title;
            ...
            $message = __('Floor plan request successfully!');
    ...
    {
        $requestProperties = PropertyRequest::accepted()->with('country', 'city', 'propertyType')->paginate(getPaginate());
        return $this->successResponse($requestProperties);
    }

    public function propertyRequestDetails($id)
    {
        $propertyRequest = PropertyRequest::accepted()->with('country', 'city', 'propertyType')->where('id', $id)->first();
        return $this->successResponse($propertyRequest);
    ...
    public function marketing()
    {
        try {
            ...
            $marketingServiceElements = getContent('marketing_service.element', null, false, true);
            ...
            foreach ($marketingServiceElements as $element) {
                $elements[] = [
                    'title' => $element->data_values->title,
                    'title_ar' => $element->data_values->title_ar,
                    'description' => $element->data_values->description,
                    'description_ar' => $element->data_values->description_ar,
                    'image' => getImage('assets/images/frontend/marketing_service/' . $element->data_values->image, '380x235'),
                ];
            }
    ...
    public function eventBanner(){
        ...
        $data["banner_section"] =[
            "title"  => $eventBannerContent->data_values->title,
            "title_ar"  => $eventBannerContent->data_values->title_ar,
            "image"  =>  getImage('assets/images/frontend/event_banner/' . $eventBannerContent->data_values->image, '1900x250'),
        ];


        return $this->successResponse($data, 'auction banner retrieve');
    }
    ...
    public function aiService()
    {
        ...
        $data['title'] = $aiBannerContent->data_values->title;
        $data['title_ar'] = $aiBannerContent->data_values->title_ar;
        $data['image'] = getImage('assets/images/frontend/ai_banner/' . $aiBannerContent->data_values->image);
    ...
    {
        $validator = Validator::make($request->all(),[
            'identify_the_applicant' => 'nullable|string|max:255',
            'fp_name' => 'nullable|string|max:255',
            'fp_mobile_number' => 'nullable|string|max:20',
            'fp_id_number' => 'nullable|string|max:50',
            'fp_characteristics_of_the_tenant' => 'nullable|string',
            'sp_name' => 'nullable|string|max:255',
            'sp_mobile_number' => 'nullable|string|max:20',
            'sp_id_number' => 'nullable|string|max:50',
            'sp_characteristics_of_the_lessor' => 'nullable|string',
            'determine_whether_the_tenant' => 'nullable|string|max:255',
            'building_number' => 'nullable|string|max:50',
            'postal_code' => 'nullable|string|max:10',
            'additional_number' => 'nullable|string|max:50',
            'city' => 'nullable|string|max:255',
            'national_address' => 'nullable|string|max:255',
            'is_correct' => 'nullable|string|max:255',
        ]);
        ...
        // Filter out the fields that should not be stored
        $data = $request->except(['paper_contract', 'sp_id_photo', 'fp_id_photo']);

        // Create a new AiService record
        $aiService = AiService::create($data);


        if ($request->hasFile('paper_contract')) {
            $file = $request->file('paper_contract');
            $filename = $aiService->id . '_paper_contract.' . $file->getClientOriginalExtension();
            $file->move(public_path(getFilePath('ai_service')), $filename);
            $aiService->paper_contract = $filename;
            $aiService->save();
        }

        if ($request->hasFile('sp_id_photo')) {
            $file = $request->file('sp_id_photo');
            $filename = $aiService->id . '_sp_id_photo.' . $file->getClientOriginalExtension();
            $file->move(public_path(getFilePath('ai_service')), $filename);
            $aiService->sp_id_photo = $filename;
            $aiService->save();
        }

        if ($request->hasFile('fp_id_photo')) {
            $file = $request->file('fp_id_photo');
            $filename = $aiService->id . '_fp_id_photo.' . $file->getClientOriginalExtension();
            $file->move(public_path(getFilePath('ai_service')), $filename);
            $aiService->fp_id_photo = $filename;
            $aiService->save();
        }
        ...
        public function foreignOwnershipBanner()
    {

        try {
            $foreign_ownership_banner = getContent('foreign_ownership_banner.content', true);
            $data['title'] = $foreign_ownership_banner->data_values->title;
            $data['title_ar'] = $foreign_ownership_banner->data_values->title_ar;
            $data['image'] = getImage('assets/images/frontend/foreign_ownership_banner/' . $foreign_ownership_banner->data_values->image);
            return $this->successResponse($data);
        }catch (Exception $exp){
            return $this->serverError('Something went wrong');
        }
    ...
    public function foreignPropertyDevelopment()
    {
        try {
            $foreign_property_development = getContent('foreign_property_development.content', true);

            $data['title'] = $foreign_property_development->data_values->title;
            $data['title_ar'] = $foreign_property_development->data_values->title_ar;
            $data['description'] = $foreign_property_development->data_values->description;
            $data['description_ar'] = $foreign_property_development->data_values->description_ar;
            $data['top_image'] = getImage('assets/images/frontend/foreign_property_development/' . $foreign_property_development->data_values->image);
            $data['bottom_image'] = getImage('assets/images/frontend/foreign_property_development/' . $foreign_property_development->data_values->image);
    ...
    public function foreignWeOfferYou()
    {
        try {

            $foreignWeOfferYouContent = getContent('foreign_we_offer_you.content', true);
            $foreignWeOfferYouElements = getContent('foreign_we_offer_you.element', null, false, true);

            $header = [
                'title' => $foreignWeOfferYouContent->data_values->header,
                'title_ar' => $foreignWeOfferYouContent->data_values->header_ar
            ];

            $services = [];

            foreach ($foreignWeOfferYouElements as $element) {
                $services[] = [
                    'title' => $element->data_values->title,
                    'title_ar' => $element->data_values->title_ar,
                    'description' => $element->data_values->description,
    ...
    public function foreignOurService()
    {
        ...
        try {
            ...
            $data['header'] = [
                'title' => $foreign_our_service_content->data_values->header,
                'title_ar' => $foreign_our_service_content->data_values->header_ar
            ];
            $data['services'] = [];
    ...
    public function foreignWhyChooseUs()
    {
        ...
        try {
            ...
            $data['header'] = [
                'title' => $foreign_why_choose_content->data_values->header,
                'title_ar' => $foreign_why_choose_content->data_values->header_ar,
                'image' => getImage('assets/images/frontend/foreign_why_choose_us/' . $foreign_why_choose_content->data_values->image)
            ];
    ...
    public function foreignOwnershipRequest(Request $request)
    {
        $validator = Validator::make($request->all(), [
        ...
        try {
            $foreignOwnership = new \App\Models\ForeignOwnershipRequest();
            ...
            $message =  __('Foreign ownership application submitted successfully!');
